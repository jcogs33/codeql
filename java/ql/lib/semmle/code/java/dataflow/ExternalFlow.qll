...
//   // exists(int n, string p | c.getParameterType(n).getErasure().toString() = p | i = n and result = p) // ! me
//   // result = c.getParameterType(i).getErasure().toString() // ! ed
// }
/**
...
string paramsString(Callable c) {
  ...
  // result = "(" + concat(int i | | paramsStringPart(c, i), "," order by i) + ")" // ! me
  result =
    "(" + concat(int i | | c.getParameterType(i).getErasure().toString(), "," order by i) + ")" // ! me/ed
}
